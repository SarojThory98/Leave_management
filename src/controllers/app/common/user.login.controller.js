	...
	try {
		...
		bcrypt.compare(password, findUser.password, (err, result) => {
			if (result) {
				...
				let token = jwt.sign({userID: findUser._id}, SECRET_KEY, {expiresIn: commonConstants.JWT_EXPIRY});
				return response.success(res, API_MESSAGE.LOGIN.LOGIN_SUCCESS, [findUser, {token: token}]);
			} else {
