...
const leaveRequestList = async (req, res) => {
	try {
		const userLeaveRequestList = await leaveRequest.find({[COMMON_MODEL_KEYS.USER_ID]: new Types.ObjectId(req.userId.user._id)});
		if (userLeaveRequestList && userLeaveRequestList.length) {
